...
fn internal_extract_unique_filter(value_map: ParsedInputMap<'_>, model: &ModelRef) -> QueryGraphBuilderResult<Filter> {
    let filters = value_map
        .into_iter()
        .map(|(field_name, value): (String, ParsedInputValue<'_>)| {
...
fn extract_relation_filters(field: &RelationFieldRef, value: ParsedInputValue<'_>) -> QueryGraphBuilderResult<Vec<Filter>> {
    match value {
