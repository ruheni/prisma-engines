...
pub(crate) fn model_object_type<'a>(ctx: &mut BuilderContext<'a>, model: &ModelRef) -> ObjectType<'a> {
    ObjectType {
...
fn compute_model_object_type_fields<'a>(ctx: &mut BuilderContext<'a>, model: &ModelRef) -> Vec<OutputField<'a>> {
    model
        .fields()
        .filter_all(|_| true)
        .into_iter()
